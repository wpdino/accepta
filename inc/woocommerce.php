 ...
 * @return void
 */
function accepta_woocommerce_scripts() {
	...
	wp_add_inline_style( 'accepta-woocommerce-style', $inline_font );
}
add_action( 'wp_enqueue_scripts', 'accepta_woocommerce_scripts' );
...
if ( ! function_exists( 'accepta_woocommerce_cart_link_fragment' ) ) {
	...
	function accepta_woocommerce_cart_link_fragment( $fragments ) {
		ob_start();
		accepta_woocommerce_cart_link();
		$fragments['a.cart-contents'] = ob_get_clean();

		return $fragments;
	}
}
add_filter( 'woocommerce_add_to_cart_fragments', 'accepta_woocommerce_cart_link_fragment' );
...
if ( ! function_exists( 'accepta_woocommerce_cart_link' ) ) {
	 ...
	 * @return void
	 */
	function accepta_woocommerce_cart_link() {
		?>
		<a class="cart-contents" href="<?php echo esc_url( wc_get_cart_url() ); ?>" title="<?php esc_attr_e( 'View your shopping cart', 'accepta' ); ?>">
			<?php
			$item_count_text = sprintf(
				/* translators: number of items in the mini cart. */
				_n( '%d item', '%d items', WC()->cart->get_cart_contents_count(), 'accepta' ),
